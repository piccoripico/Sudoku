...
| デスクトップ表示 | モバイル表示 |
| --- | --- |
| <img src="./docs/screenshot_desktop_ja.jpeg" alt="デスクトップで見た数独画面" width="520" /> | <img src="./docs/screenshot_mobile_ja.jpeg" alt="モバイルで見た数独画面" width="160" /> |
...
- **ライト / ダークテーマ**: 表示を「システム設定」「ライト」「ダーク」から選べます。「システム設定」ではOSのカラーモードに自動追従します。
- **設定保存はオプトイン**: 初期状態ではブラウザに設定を保存しません。「設定保存」をONにした場合だけ、ヒント数、シード、言語、表示テーマをブラウザのローカルストレージに保存し、次回起動時に復元します。OFFに戻すと保存済み設定を削除します。
